...
class ModelManager:
    ...
    def load_llm_model(self) -> bool:
        ...
        try:
            ...
            # 为DialoGPT添加pad_token
            if "dialogpt" in MODEL_CONFIG['llm_model'].lower():
            ...
            # 根据模型类型选择合适的模型类
            if "dialogpt" in MODEL_CONFIG['llm_model'].lower() or "gpt" in MODEL_CONFIG['llm_model'].lower():
                # 对话生成模型使用CausalLM
      ...
      def analyze_text_risk(self, text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if "gemma" in model_name:
                ...
                # DialoGPT的对话生成接口
                inputs = self.llm_tokenizer.encode(prompt + self.llm_tokenizer.eos_token, return_tensors="pt")
                if self.device == "cuda":
                ...
                with torch.no_grad():
                    outputs = self.llm_model.generate(
                        inputs,
                        max_new_tokens=256,  # DialoGPT适合较短的回复
                        temperature=MODEL_CONFIG['temperature'],
                        do_sample=True,
                        pad_token_id=self.llm_tokenizer.eos_token_id,
